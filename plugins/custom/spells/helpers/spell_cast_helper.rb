  ...
  module Custom
    ...
    def self.cast_equip_armor(caster, spell)
      ...
      if succeeds == "%xgSUCCEEDS%xn"
        caster.update(armor_name: armor)
        FS3Combat.emit_to_combat caster.combat, t('custom.casts_spell', :name => caster.name, :spell => spell, :succeeds => succeeds)
      else
        FS3Combat.emit_to_combat caster.combat, t('custom.casts_spell', :name => caster.name, :spell => spell, :succeeds => succeeds)
      end
      FS3Combat.set_action(client, caster, caster.combat, caster, FS3Combat::SpellAction, "")
    end
    ...
    def self.cast_equip_armor_specials(caster, spell)
      ...
      client = Login.find_client(caster)
      if succeeds == "%xgSUCCEEDS%xn"
        caster.update(armor_specials: armor_specials)
    ...
    def self.cast_equip_weapon(caster, spell)
      ...
      caster.update(weapon_name: weapon)
        weapon_type = FS3Combat.weapon_stat(caster.weapon, "weapon_type")
      armor = Global.read_config("spells", spell, "armor")
      ...
      if armor
        ...
        FS3Combat.emit_to_combatant caster, t('custom.target_aoe')
      elsif weapon_type == "Supressive"
        FS3Combat.emit_to_combat caster.combat, t('custom.will_cast_spell', :name => caster.name, :spell => spell)
        FS3Combat.emit_to_combatant caster, t('custom.target_suppress')
      else
        FS3Combat.emit_to_combat caster.combat, t('custom.will_cast_spell', :name => caster.name, :spell => spell)
        FS3Combat.emit_to_combatant caster, t('custom.target_attack')
      end
    end
    ...
    def self.cast_equip_weapon_specials(caster, spell)
      ...
      client = Login.find_client(caster)
      if succeeds == "%xgSUCCEEDS%xn"
        caster.update(weapon_specials: weapon_specials)
        FS3Combat.emit_to_combat caster.combat, t('custom.casts_spell', :name => caster.name, :spell => spell)
      else
        FS3Combat.emit_to_combat caster.combat, t('custom.casts_spell', :name => caster.name, :spell => spell, :succeeds => succeeds)
      end
    ...
    def self.cast_stun_spell(caster, spell)
      client = Login.find_client(caster)
      FS3Combat.emit_to_combat caster.combat, t('custom.will_cast_spell', :name => caster.name, :spell => spell)
      FS3Combat.emit_to_combatant caster, t('custom.target_stun')
    end
