  ...
  module Custom
    class GetKillsCmdHandler
      def handle(request)
        ...
        {
          scoreboard: scoreboard.each_with_index.map { |(char, victories), i| { char: { name: char.name, military_name: char.military_name, icon: WebHelpers.icon_for_char(char) }, kill_count: victories.count, index: i + 1 } },
