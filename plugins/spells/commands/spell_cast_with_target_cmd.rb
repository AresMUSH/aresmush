  ...
  module Spells
    class SpellCastWithTargetCmd
      ...
      def check_errors
        ...
        return t('Spells.already_cast') if Custom.already_cast(enactor)
        return t('Spells.no_target') if !require_target
        return t('Spells.not_in_combat') if !enactor.combat
        return nil
      end
        ...
        #Reading Config Files
          ...
          if damage_inflicted
            if succeeds == "%xgSUCCEEDS%xn"
              ...
              FS3Combat.emit_to_combat enactor.combat, t('Spells.cast_damage', :name => enactor.name, :spell => self.spell, :succeeds => succeeds, :target => target.name, :damage_desc => self.spell.downcase)
            else
          ...
          if heal_points
            if succeeds == "%xgSUCCEEDS%xn"
              ...
              if (wound)
                ...
                FS3Combat.emit_to_combat enactor.combat, t('Spells.cast_heal', :name => enactor.name, :spell => self.spell, :succeeds => succeeds, :target => target.name, :points => heal_points)
              else
                FS3Combat.emit_to_combat enactor.combat, t('Spells.cast_heal_no_effect', :name => enactor.name, :spell => self.spell, :succeeds => succeeds, :target => target.name)
              end
            else
                FS3Combat.emit_to_combat enactor.combat, t('Spells.casts_spell', :name => enactor.name, :spell => spell, :succeeds => succeeds)
            end
          ...
          if is_revive
            if (!target.combatant.is_ko)
              ...
              FS3Combat.emit_to_combat enactor.combat, t('Spells.cast_revive', :name => enactor.name, :spell => self.spell, :succeeds => succeeds, :target => target.name)
              FS3Combat.emit_to_combatant target.combatant, t('Spells.been_revived', :name => enactor.name)
            else
              FS3Combat.emit_to_combat enactor.combat, t('Spells.casts_spell', :name => enactor.name, :spell => spell, :succeeds => succeeds)
            end
            FS3Combat.set_action(client, enactor, enactor.combat, enactor.combatant, FS3Combat::SpellAction, "")
          ...
          if defense_mod
            if succeeds == "%xgSUCCEEDS%xn"
              ...
              FS3Combat.emit_to_combat enactor.combat, t('Spells.cast_mod', :name => enactor.name, :target => target.name, :spell => self.spell, :succeeds => succeeds, :mod => defense_mod, :type => "defense", :total_mod => target.combatant.defense_mod)
            else
              FS3Combat.emit_to_combat enactor.combat, t('Spells.casts_spell', :name => enactor.name, :spell => spell, :succeeds => succeeds)
            end
            FS3Combat.set_action(client, enactor, enactor.combat, enactor.combatant, FS3Combat::SpellAction, "")
          ...
          if attack_mod
            if succeeds == "%xgSUCCEEDS%xn"
              ...
              FS3Combat.emit_to_combat enactor.combat, t('Spells.cast_mod', :name => enactor.name, :target => target.name, :spell => self.spell, :succeeds => succeeds, :mod => attack_mod, :type => "attack", :total_mod => target.combatant.attack_mod)
            else
              FS3Combat.emit_to_combat enactor.combat, t('Spells.casts_spell', :name => enactor.name, :spell => spell, :succeeds => succeeds)
            end
            FS3Combat.set_action(client, enactor, enactor.combat, enactor.combatant, FS3Combat::SpellAction, "")
          ...
          if spell_mod
            if succeeds == "%xgSUCCEEDS%xn"
              ...
              FS3Combat.emit_to_combat enactor.combat, t('Spells.cast_mod', :name => enactor.name, :target => target.name, :spell => self.spell, :succeeds => succeeds, :mod => spell_mod, :type => "spell", :total_mod => target.combatant.spell_mod)
            else
              FS3Combat.emit_to_combat enactor.combat, t('Spells.casts_spell', :name => enactor.name, :spell => spell, :succeeds => succeeds)
            end
            FS3Combat.set_action(client, enactor, enactor.combat, enactor.combatant, FS3Combat::SpellAction, "")
          ...
          if stance
            if succeeds == "%xgSUCCEEDS%xn"
              ...
              FS3Combat.emit_to_combat enactor.combat, t('Spells.cast_stance', :name => enactor.name, :target => target.name, :spell => self.spell, :succeeds => succeeds, :stance => stance)
            else
              FS3Combat.emit_to_combat enactor.combat, t('Spells.casts_spell', :name => enactor.name, :spell => spell, :succeeds => succeeds)
            end
            FS3Combat.set_action(client, enactor, enactor.combat, enactor.combatant, FS3Combat::SpellAction, "")
