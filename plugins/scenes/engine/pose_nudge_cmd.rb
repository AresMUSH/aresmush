  ...
  module Scenes
    class PoseNudgeCmd
      ...
      def check_status
        ...
        return t('scenes.pose_nudge_options') if self.option.validate
      end
      
      def handle
        if (self.muted)
          enactor.update(pose_nudge_muted: true)
          client.emit_success t('scenes.pose_nudge_muted')
        else
          enactor.update(pose_nudge_muted: false)
          enactor.update(pose_nudge: self.option.is_on?)
          client.emit_success t("scenes.pose_nudge_#{self.option.is_on? ? 'on' : 'off'}")
        end
      end
    end
  end
