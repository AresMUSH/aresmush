    ...
    module AbilityPointCounter
      def self.total_points(char)
        return self.points_on_attrs(char) + self.points_on_action(char) +
           ...
           self.points_on_specialties(char) + self.points_on_advantages(char) + self.points_on_spells(char)
      end
      ...
      def self.points_on_spells(char)
        cost = Global.read_config("fs3skills","spell_cost")
        char.fs3_spells.inject(0) { |count, a| count + (a.level * cost) }
      end
