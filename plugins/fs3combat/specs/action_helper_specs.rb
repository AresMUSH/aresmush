    ...
    describe FS3Combat do
      ...
      describe :determine_damage do
        before do
          ...
          allow(FS3Combat).to receive(:rand) { 0 }
          allow(Global).to receive(:read_config).with("fs3combat", "damage_table") { { "GRAZE" => 20, "FLESH" => 70, "MODERATE" => 80, "IMPAIR" => 100 } }
        ...
        describe "random damage" do
          ...
          it "should roll a moderate wound" do
            allow(FS3Combat).to receive(:rand) { 70 }
            expect(FS3Combat.determine_damage(@combatant, "Chest", "Knife")).to eq "MODERATE"
          end
