module AresMUSH
  module Chargen
    ...
    def self.bg_app_review(char)
      puts "BACKGROUND SIZE #{char.background.size}"
      error = char.background.to_s.empty? ? t('chargen.not_set') : t('chargen.ok')
      error = char.background.size > 4000 ? "%xr< Backgrounds should be 4,000 characters or less. Currently #{char.background.size.to_s.reverse.gsub(/(\d{3})(?=\d)/, '\\1,').reverse} characters. %xn >" : t('chargen.ok')
