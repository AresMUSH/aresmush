      ...
      def self.export
        begin
          ...
          Scene.all.select { |s| s.shared }.sort_by { |s| s.icdate }.each do |s|
            ...
            index["Scenes"][page_name] = s.date_title
          ...
          Chargen.approved_chars.select { |c| !c.is_admin? && c.is_approved? }.sort_by { |c| c.name }.each do |c|
          ...
          index.each do |section, data|
            File.open(File.join(export_path, "#{section}.html"), 'w') do |f|
              text = ""
              if (section == 'Wiki')
              ...
              f.puts format_wiki_template(text, section)
              
            end
          end
          
          File.open(File.join(export_path, "characters.html"), 'w') do |f|
            data = Chargen.approved_chars.map { |c| 
              {
                name: c.name,
