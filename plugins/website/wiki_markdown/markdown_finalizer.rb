    ...
    class MarkdownFinalizer
      def self.process(markdown)
        ...
        begin
            ...
            if ( line =~ /^(<p>)?(<br>)?`/)
              ...
              if (tab_name)
               if (tabs[tab_name])
                  ...
                  html << line
                  Global.logger.warn("Invalid tab name in markdown: #{tab_name}")
                end
              else
                html << "#{line}\n"
              end
          ...
          Global.logger.error "Error parsing markdown: error=#{e} backtrace=#{e.backtrace[0,10]}"
          "There was a problem processing your markdown text."
        end
      end
