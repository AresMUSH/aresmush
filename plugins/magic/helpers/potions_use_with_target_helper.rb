module AresMUSH
  module Magic
    ...
    def self.potion_inflict_damage_with_target(caster, target, spell)
      ...
      return t('magic.cant_heal_dead') if (target.dead)
      FS3Combat.inflict_damage(target, damage_inflicted, damage_desc)
      FS3Combat.emit_to_combat caster.combat, t('magic.potion_damage', :name => caster.name, :potion => spell, :target => target.name)
    end
    ...
    def self.potion_heal_with_target(caster, target, spell)
      ...
      if (wound)
        ...
        FS3Combat.emit_to_combat caster.combat, t('magic.potion_heal_with_target', :name => caster.name, :potion => spell, :target => target.name, :points => heal_points)
      else
        FS3Combat.emit_to_combat caster.combat, t('magic.potion_heal_no_effect_with_target', :name => caster.name, :potion => spell, :target => target.name)
      end
    end
    ...
    def self.potion_non_combat_heal_with_target(caster, target, spell)
      ...
      if (wound)
        FS3Combat.heal(wound, heal_points)
        client.emit t('magic.potion_heal_with_target', :name => caster.name, :potion => spell, :target => target.name, :points => heal_points)
      else
        client.emit t('magic.potion_heal_no_effect_with_target', :name => caster.name, :potion => spell, :target => target.name)
      end
    end

    def self.potion_revive(caster_combat, target_combat, spell)
      target_combat.update(is_ko: false)
      FS3Combat.emit_to_combat caster_combat.combat, t('magic.potion_res', :name => caster_combat.name, :potion => spell, :target => target_combat.name)
      FS3Combat.emit_to_combatant target_combat, t('magic.been_resed', :name => caster_combat.name)
    end

    # def self.cast_revive(caster, target, target_combat, spell)
    #   succeeds = Magic.roll_combat_spell_success(caster, spell)
    #   if succeeds == "%xgSUCCEEDS%xn"
    #     Magic.undead(target)
    #     FS3Combat.emit_to_combat caster.combat, t('magic.cast_res', :name => caster.name, :spell => spell, :succeeds => succeeds, :target => target.name)
    #     FS3Combat.emit_to_combatant target_combat, t('magic.been_resed', :name => caster.name)
    #   else
    #     FS3Combat.emit_to_combat caster.combat, t('magic.casts_spell', :name => caster.name, :potion => spell, :succeeds => succeeds)
    #   end
    # end
    ...
    def self.potion_defense_mod_with_target(caster,  target_combat, spell)
      ...
      FS3Combat.emit_to_combat caster.combat, t('magic.potion_mod_with_target', :name => caster.name, :target => target_combat.name, :potion => spell, :mod => defense_mod, :type => "defense", :total_mod => target_combat.defense_mod)
    end
    ...
    def self.potion_attack_mod_with_target(caster, target_combat, spell)
      ...
      FS3Combat.emit_to_combat caster.combat, t('magic.potion_mod_with_target', :name => caster.name, :target => target_combat.name, :potion => spell, :mod => attack_mod, :type => "attack", :total_mod => target_combat.attack_mod)
    end
    ...
    def self.potion_spell_mod_with_target(caster, target_combat, spell)
      ...
      FS3Combat.emit_to_combat caster.combat, t('magic.potion_mod_with_target', :name => caster.name, :target => target_combat.name, :potion => spell, :mod => spell_mod, :type => "spell", :total_mod => target_combat.spell_mod)
    end
    ...
    def self.potion_stance_with_target(caster, target_combat, spell)
      ...
      FS3Combat.emit_to_combat caster.combat, t('magic.potion_stance_with_target', :name => caster.name, :target => target_combat.name, :potion => spell, :stance => stance)
    end
    ...
    def self.potion_equip_weapon_with_target(enactor, caster_combat, target_combat, spell)
      ...
      if armor
        ...
        FS3Combat.emit_to_combat caster_combat.combat, t('magic.use_potion_with_target', :name => caster_combat.name, :potion => spell, :target => target_combat.name)
      end
