module AresMUSH
  module Magic
    ...
    def self.roll_noncombat_spell(caster, spell, mod = nil)
      ...
      total_mod = mod.to_i + cast_mod + item_spell_mod.to_i + level_mod

      Global.logger.info "#{caster.name} rolling #{skill} to cast #{spell}. Level Mod=#{level_mod} Mod=#{mod} Item Mod=#{item_spell_mod} Off-school cast mod=#{cast_mod} total=#{total_mod}"
