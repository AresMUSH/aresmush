    ...
    describe TeleportCmd do
      ...
      describe :find_destination do
        it "should find the room when the destination is a char" do
          ...
          SingleTargetFinder.should_receive(:find).with("somewhere", Character) { FindResult.new(other_char, nil) }
          SingleTargetFinder.should_not_receive(:find).with("somewhere", Room)
        ...
        it "should find the room when the destination is a room" do
          ...
          SingleTargetFinder.should_receive(:find).with("somewhere", Room) { FindResult.new(room, nil) }
          handler.find_destination.should eq room
        end
      ...
      describe :find_targets do
        ...
        it "should return the matching char if there is one" do
          ...
          SingleTargetFinder.should_receive(:find).with("someone", Character) { FindResult.new(other_char, nil) }
        ...
        it "should return nil if nothing found" do
          handler.stub(:name) { "someone" }
          SingleTargetFinder.should_receive(:find).with("someone", Character) { FindResult.new(nil, "error") }
