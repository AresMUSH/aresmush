  ...
  module Api
    describe MasterRegisterCmdHandler do
      ...
      it "should fail if game already registered" do
        cmd = ApiCommand.create_from("register somewhere.com||101||A MUSH||Social||A cool MUSH||http://www.somewhere.com||no")
      ...
      it "should fail if the port is not a number" do
        cmd = ApiCommand.create_from("register somewhere.com||x||A MUSH||Social||A cool MUSH||http://www.somewhere.com||no")
        response = Global.api_router.route_command(-1, cmd)
        check_response(response, ApiResponse.error_status, "api.invalid_port")
      end
      
      it "should fail if the category is not valid" do
        cmd = ApiCommand.create_from("register somewhere.com||101||A MUSH||x||A cool MUSH||http://www.somewhere.com||no")
        response = Global.api_router.route_command(-1, cmd)
        check_response(response, ApiResponse.error_status, "api.invalid_category")
      end
      
      it "should create the game" do
        cmd = ApiCommand.create_from("register somewhere.com||101||A MUSH||Social||A cool MUSH||http://www.somewhere.com||yes")
