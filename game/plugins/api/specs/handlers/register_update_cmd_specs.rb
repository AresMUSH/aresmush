  ...
  module Api
    describe RegisterUpdateCmdHandler do
      ...
      it "should fail if game not registered" do
        cmd = ApiCommand.create_from("register/update somewhere.com||101||A MUSH||Social||A cool MUSH||http://www.somewhere.com||false")
      ...
      it "should fail if the port is not a number" do
        cmd = ApiCommand.create_from("register/update somewhere.com||x||A MUSH||Social||A cool MUSH||http://www.somewhere.com||false")
        response = Global.api_router.route_command(1, cmd)
        check_response(response, ApiResponse.error_status, "api.invalid_port")
      end
      
      it "should fail if the category is not valid" do
        cmd = ApiCommand.create_from("register/update somewhere.com||101||A MUSH||x||A cool MUSH||http://www.somewhere.com||false")
        response = Global.api_router.route_command(1, cmd)
      ...
      it "should update the game" do
        cmd = ApiCommand.create_from("register/update somewhere.com||101||A MUSH||Social||A cool MUSH||http://www.somewhere.com||true")
        game = double
        game.stub(:game_id) { 1 }
