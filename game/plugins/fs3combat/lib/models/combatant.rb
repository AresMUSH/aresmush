module AresMUSH
  ...
  class Combatant
    ...
    def roll_ability(ability, mod = 0)
      if (is_npc?)
        result = FS3Skills::Api.one_shot_die_roll(self.npc_skill + mod)
      else
        params = FS3Skills::RollParams.new(ability, mod)
        result = FS3Skills::Api.one_shot_roll(nil, self.character, params)
      end
      result[:successes]
    end
    ...
    def poss_pronoun
      is_npc? ? t('demographics.other_possessive') : Demographics::Api.possessive_pronoun(self.character)
    end
